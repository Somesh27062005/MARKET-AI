...
def lead_node(state: LeadState) -> dict:
    ...
    prompt = (
        ...
        f"1. Evaluate budget, authority, need, and timeline (BANT). For each provide a score (0-100), assessment (10-15 words), and evidence (10-15 words).\n"
        f"2. Assess buying intent, listing exactly 3 specific intent signals and their strength (High/Medium/Low).\n"
        f"3. Identify exactly 3 potential risk factors (risk: 6-10 words, impact: High/Medium/Low, mitigation: 10-15 words).\n"
        f"4. Recommend exactly 3 actions for {co_name}'s sales team (action: 8-12 words, priority: High/Medium/Low, timeline: 2-4 words) and specify the single next_best_action (8-12 words).\n"
        f"5. Calculate an overall lead_score (weighted: Budget 30%, Authority 25%, Need 30%, Timeline 15%) and conversion_probability.\n"
        f"6. Determine lead temperature (Hot/Warm/Cold), priority_level (Critical/High/Medium/Low), and CRM readiness.\n"
        f"7. Provide a detailed score breakdown for exactly 4 factors (BANT): factor (e.g. 'Budget Fit'), score (0 to max), max, and rationale (8-12 words).\n"
        f"8. Provide a concise 2-sentence qualification_summary (25-35 words).\n"
        f"{grounding}"
    )
